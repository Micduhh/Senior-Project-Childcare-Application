...
namespace DatabaseController {
    class GuardianInfoDB {
        ...
        public String GetTotalDue(String parentID)
        {
            String familyID = parentID.Remove(parentID.Length - 1);
            String query = "";
            ...
            String regQuery = "SELECT RegularTotal FROM Family WHERE Family_ID = '" + familyID + "';";
            String campQuery = "SELECT CampTotal FROM Family WHERE Family_ID = '" + familyID + "';";
            String miscQuery = "SELECT MiscTotal FROM Family WHERE Family_ID = '" + familyID + "';";
            
            SQLiteCommand regcmd = new SQLiteCommand(regQuery, dbCon);
            SQLiteCommand campcmd = new SQLiteCommand(campQuery, dbCon);
            SQLiteCommand misccmd = new SQLiteCommand(miscQuery, dbCon);

            string regDue = ExecuteQuery(regcmd);
            string campDue = ExecuteQuery(campcmd);
            string miscDue = ExecuteQuery(misccmd);

            regular = decimal.Parse(regDue);
            camp = decimal.Parse(campDue);
            misc = decimal.Parse(miscDue);

            totalDue = regular + camp + misc;

            return $"${totalDue.ToString()}";
        }
        
        private string ExecuteQuery(SQLiteCommand command)
        {
            string curDue = string.Empty;

            try
            {
                dbCon.Open();
                curDue = String.Format("{0:0.00}", command.ExecuteScalar());
                dbCon.Close();
            }
            catch (System.Data.SQLite.SQLiteException)
            {
