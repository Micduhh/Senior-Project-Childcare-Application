...
namespace ChildcareApplication.AdminTools {
    class RegExpressions {
        ...
        public static bool RegexAddress(string data) {
             ...
             * 123 Road Road
             */
            Regex regex = new Regex(@"(^(\d{1,6}\s)(N|S|E|W)?(.)?(\s)?([a-zA-Z]+\s?)+((RD|rd|ST|st|BLVD|blvd|AVE|ave|CT|ct|LN|ln|WAY|way|PL|pl|ALY|aly|Road|Street|Boulevard|Avenue|Court|Lane|Place|Alley)?\.?)$)");
